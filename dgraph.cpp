...
Local Variable: int i, int j, int k
*/
...
/*
Purpose: which will enter the given visit number for a given vertex
this is to indicate the order in which vertices were visited.
Local Variables: int i
Parameters: int num, char index
*/
void dgraph::visit(int num, char index)
{
 for (int i = 0; i < countUsed; i++)
 {
  if(index == Gtable[i].vertexName) // If the char param is the same as the vertex
   { 
    Gtable[i].visit = num; // Give the vertex a visit number
    return;//then return 
   }
 }
}
...
/*
Purpose: which returns true if a given vertex was already visited
(0 means not visited)
Local Variables: 
Parameters: char index
*/
bool dgraph::isMarked(char index)
{
 for (int i = 0; i < countUsed; i++)
  {
   if (index == Gtable[i].vertexName) // If the char param is the same as the vertex
   {
    if(Gtable[i].visit > 0) // If the visit number is not zero
     return true;
    else
     return false;
   }
